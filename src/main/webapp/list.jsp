...
<body>
	...
	<div class="box">
		<p class="box1"><form action="./list" method="POST" style="display: inline"><!-- フォームの遷移先とメソッドの指定 -->
			...
			for(QuestionsBean questionsData : questionsList){ %><!-- 配列questionsListから要素取り出しQuestionsBean型変数queDataに格納、配列の分だけ繰返し -->
			
			
			<label for="question">問題 
				<input type="text" name="questionsId" value ="<%=questionsData.getId()%>" class="QidTextBox"  readonly> :<!-- テキストボックスにquestionsListから取得したquestionsIdを表示 -->
				<input type="text" name="question" id="question" value ="<%=questionsData.getQuestion()%>" class="QTextBox" readonly><!-- 同様に取得したQuestionを表示 -->
			</label>	
		</form>
				
		<form action="./edit" method="POST"  style="display: inline"><!-- 遷移先と送信メソッドの指定、改行の消去 -->
			<!-- name="id"のテキストボックスが配列から取得したidを保持した状態で編集画面に遷移 -->
			<input type="hidden" name="questionsId" value ="<%=questionsData.getId()%>" >
		...
		<%int i = 1;//int型変数iを1の状態で宣言
			for(AnswersBean answersData : answersList){	%><!-- 配列ansListから要素を取り出しAnswersBean型変数ansDataに格納、配列の分だけ繰返し -->
				<%if(answersData.getQuestions_id() == questionsData.getId() ){ %><!-- ansDataが取得したquestions_idとqueDataが取得したidが一致 -->
							
		<label>	答え<%= i++ %>:<!-- 変数iにプラス１した状態で表示 -->
			<input type="text" name="answer" value ="<%=answersData.getAnswer()%>" class="ATextBox" readonly><!-- if文がtrueのときansDataが取得したanswerを表示 -->
		</label>	
